...
public class APIServer {
	public APIServer(Database database) {
		...
		Spark.post("/school/v1/units", (req, res) -> {
			try {
				System.out.println("  POST: /school/units?group=" + req.queryParams("group") +
						"&units=" + req.queryParams("units") + "&age=" + req.queryParams("age") + "&taste=" + req.queryParams("taste"));
				database.addUnits(req.queryParams("group"),
		...
		Spark.post("/school/v1/units/del", (req, res) -> {
			try {
				System.out.println("DELETE: /school/units?group=" + req.queryParams("group"));
				database.deleteLastUnit(req.queryParams("group"));
			} catch (Exception e) {
				e.printStackTrace();
				res.status(400);
			}
